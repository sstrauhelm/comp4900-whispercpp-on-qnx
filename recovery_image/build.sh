...
function build_sources() {
    ...
    mkimage -A arm -O linux -T script -C none -d "${SCRIPTS_PATH}/boot_script.txt" "${BUILD_PATH}/boot.scr.uimg"
}
...
function create_image() {
    ...
    sudo parted -s "${IMAGE_PATH}" -- unit KiB mkpart primary ext2 45056 -1s
    ...
    sudo mkfs.ext3 "/dev/mapper/${LOOPDEV}p2"
    sudo parted "${IMAGE_PATH}" print
    ...
    sudo cp -rv "${SOURCES_PATH}/firmware-${RPI_FIRMWARE_VER}/boot/"{bootcode.bin,bcm2711-rpi-4-b.dtb,fixup4.dat,fixup4cd.dat,fixup4db.dat,fixup4x.dat,start4.elf,start4cd.elf,start4db.elf,start4x.elf} /mnt/rpi/
